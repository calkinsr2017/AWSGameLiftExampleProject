...
UGameLiftDescribeGameSessions* UGameLiftDescribeGameSessions::DescribeGameSessions()
{
#if WITH_GAMELIFTCLIENTSDK
	UGameLiftDescribeGameSessions* Proxy = NewObject<UGameLiftDescribeGameSessions>();
	return Proxy;
#endif
	return nullptr;
}

EActivateStatus UGameLiftDescribeGameSessions::Activate()
{
#if WITH_GAMELIFTCLIENTSDK
	if (GameLiftClient)
	{
		...
		if (OnDescribeGameSessionsStateSuccess.IsBound() == false)
		{
			LOG_ERROR("No functions were bound to OnDescribeGameSessionsStateSuccess multi-cast delegate! Aborting Activate.");
			return EActivateStatus::ACTIVATE_NoSuccessCallback;
		}

		if (OnDescribeGameSessionsStateFailed.IsBound() == false)
		{
			LOG_ERROR("No functions were bound to OnDescribeGameSessionsStateFailed multi-cast delegate! Aborting Activate.");
			return EActivateStatus::ACTIVATE_NoFailCallback;
		}
		
		Aws::GameLift::Model::DescribeGameSessionsRequest Request;
		FString fleetID = "fleet-1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d";
		Request.SetFleetId("fleet-1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d");
		//Request.SetGameSessionId(TCHAR_TO_UTF8(*SessionID));

		Aws::GameLift::DescribeGameSessionsResponseReceivedHandler Handler;
		Handler = std::bind(&UGameLiftDescribeGameSessions::OnDescribeGameSessionsState, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);

		LOG_NORMAL("Requesting to describe game sessions");
...
void UGameLiftDescribeGameSessions::OnDescribeGameSessionsState(const Aws::GameLift::GameLiftClient* Client, const Aws::GameLift::Model::DescribeGameSessionsRequest& Request, const Aws::GameLift::Model::DescribeGameSessionsOutcome& Outcome, const std::shared_ptr<const Aws::Client::AsyncCallerContext>& Context)
{
#if WITH_GAMELIFTCLIENTSDK
	if (Outcome.IsSuccess())
	{
		LOG_NORMAL("Received OnDescribeGameSessionsState with Success outcome.");
		FString GameLiftId;
		for (const Aws::GameLift::Model::GameSession session : Outcome.GetResult().GetGameSessions())
		{
			GameLiftId = FString(session.GetGameSessionId().c_str());
		}

		OnDescribeGameSessionsStateSuccess.Broadcast(GameLiftId);
	}
	else
	{
		const FString MyErrorMessage = FString(Outcome.GetError().GetMessage().c_str());
		LOG_ERROR("Received OnDescribeGameSessionsState with failed outcome. Error: " + MyErrorMessage);
		OnDescribeGameSessionsStateFailed.Broadcast(MyErrorMessage);
	}
#endif
