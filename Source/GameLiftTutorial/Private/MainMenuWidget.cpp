...
UMainMenuWidget::UMainMenuWidget(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	OculusAppId = TextReader->ReadFile("Urls/OculusAppID.txt");
	OculusSecretId = TextReader->ReadFile("Urls/OculusSecretID.txt");
...
void UMainMenuWidget::HandleLoginUrlChange()
{
	...
	UE_LOG(LogOculus, Log, TEXT("%s   current url: %s"), __FUNCTIONW__, *BrowserUrl);

	
	if (BrowserUrl.Split("#", &LoginUrl2, &base64Json))
	{
		UE_LOG(LogOculus, Log, TEXT("%s   Coded Json: %s"), __FUNCTIONW__, *base64Json);
		...
		if(FBase64::Decode(base64Json, convertedJson))
		{
			...
			{
				UE_LOG(LogOculus, Log, TEXT("%s Json: %s"), __FUNCTIONW__, *convertedJson);

	
				code = JsonObject->GetStringField("code");
				org_scoped_id = JsonObject->GetStringField("org_scoped_id");
				UE_LOG(LogOculus, Log, TEXT("%s   Coded: %s  Org Scope Id: %s"), __FUNCTIONW__, *code, *org_scoped_id);

				TSharedRef<IHttpRequest> GetOculusRequest = HttpModule->CreateRequest();
				GetOculusRequest->OnProcessRequestComplete().BindUObject(this, &UMainMenuWidget::OnGetOculusResponseRecieved);
				GetOculusRequest->SetURL("https://graph.oculus.com/sso_authorize_code?code=" + code + "&access_token=OC|" +OculusAppId + "|" + OculusSecretId + "&org_scoped_id=" + org_scoped_id);
				GetOculusRequest->SetVerb("POST");
				GetOculusRequest->ProcessRequest();
			}
...
void UMainMenuWidget::OnGetOculusResponseRecieved(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		TSharedPtr<FJsonObject> JsonObject;
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject))
		{
			if (JsonObject->HasField(("oauth_token")) && JsonObject->HasField(("refresh_code")))
			{


				OauthToken = JsonObject->GetStringField("oauth_token");
				RefreshToken = JsonObject->GetStringField("refresh_code");
				//Pass the tokens for use with the AWS console
				UE_LOG(LogOculus, Log, TEXT("%s   oauth token: %s  RefreshToken: %s"), __FUNCTIONW__, *OauthToken, *RefreshToken);


				TSharedRef<IHttpRequest> GetPlayerDataRequest = HttpModule->CreateRequest();
				GetPlayerDataRequest->OnProcessRequestComplete().BindUObject(this, &UMainMenuWidget::OnGetOculusPlayerDataResponseRecieved);
				GetPlayerDataRequest->SetURL("https://graph.oculus.com/me?access_token=" + OauthToken + "&fields=id,alias");
				GetPlayerDataRequest->SetVerb("GET");
				GetPlayerDataRequest->ProcessRequest();
...
void UMainMenuWidget::OnGetOculusPlayerDataResponseRecieved(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject))
		{
			if (JsonObject->HasField(("id")) && JsonObject->HasField(("alias")))
			{
